...
def main(args):
    ...
    wandb.init(
        project='',
        entity='',
        name=f"sb3_PPO_{args.n_nodes}_nodes_fixed_length_{args.episode_length}_skill_{args.red_skill}_{'random' if args.random else 'static'}",
        sync_tensorboard=True,
    )

    ppo_config = pyron.load(f"{parent_dir}/configs/yawning_titan/train_config/{args.config}")

    seed=args.seed
    env_cls = network_env_cls(network, game_mode, args.random, seed, args.n_nodes, args.edge_prob)
...
if __name__ == '__main__':
    ...
    if not args.checkpoint_dir:
        ...
        args.checkpoint_dir = f"checkpoints/Sb3_PPO_YT_{args.n_nodes}_nodes_fixed_length_{args.episode_length}_skill_{args.red_skill}_{episode_type}_{time.time()}"
