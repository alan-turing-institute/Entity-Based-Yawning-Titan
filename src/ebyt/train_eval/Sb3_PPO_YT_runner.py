...
from gym.wrappers import Monitor as gym_Monitor
...
AGENTS_DIR = f"{parent_dir}/train_eval/checkpoints"
...
class YawningTitanRun:
    ...
    def __init__(
        self,
        env_cls: Type[YTGenericNetworkEnv],
        ppo_config: Dict,
        ...
        **kwargs,
        ...
        :param env_cls: The environment class to be used for training. Must be a subclass of ``YTGenericNetworkEnv``.
        :param print_metrics: Print the metrics if True.
        :param show_metrics_every: Prints the metrics every ``show_metrics_every`` time steps. 
        :param collect_additional_per_ts_data: Collects additional per-timestep data if True.
        ...
        :param render: Renders the environment during evaluation if True. 
        ...
        self.env: Optional[YTGenericNetworkEnv] = None
    ...
    def _args_dict(self):
        return {
            ...
            'env_cls': self._env_cls.__name__,
    ...
    def setup(self, new: bool = True, ppo_zip_path: Optional[str] = None):
        ...
        self.logger.debug(f'YT run  {self.uuid}: Env created')
        ...
        if self.env and self.agent and self.eval_callback:
            ...
            for i in range(self.training_runs):
                self.agent.learn(
                    total_timesteps=self.total_timesteps,
                    n_eval_episodes=self.n_eval_episodes,
                    callback=[self.eval_callback, WandbCallback()],
                )
    ...
    def evaluate(self) -> Union[tuple[float, float], tuple[List[float], List[int]]]:
        ...
        if self.agent:
            return evaluate_policy(
                self.agent, Monitor(gym_Monitor(self.env,str(self.output_dir))), n_eval_episodes=self.n_eval_episodes)
        else:
            self.logger.error(
    ...
    @classmethod
    def load(cls, path: str, env_cls: Type[YTGenericNetworkEnv]) -> YawningTitanRun:
        """
